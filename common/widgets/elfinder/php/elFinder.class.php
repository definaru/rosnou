...
class elFinder {
    ...
    protected function open($args) {
        ...
        if (!empty($args['init'])) {
            ...
            // for upload big images
            $result['uplMaxSize'] = '15M';//ini_get('upload_max_filesize');
...
//        $name   = $args['name'];
    ...
    **/
    ...
    protected function upload($args) {
        ...
        $files  = isset($args['FILES']['upload']) && is_array($args['FILES']['upload'])
            ? $args['FILES']['upload']
            : array();
...
//        }
        ...
        foreach ($files['name'] as $i => $name) {
            ...
            $name = $this->translitFilename($name); // transliteration filename
            ...
            if($file['size'] >= 1024 * 1024) {
                // resize images which more 1Mb
                $volume->resize($file['hash'], 1000, 1000, 0, 0, 'propresize');
            }
    ...
    **/
    ...
    protected function translit($string) {
        $string = \common\components\Helper::rus2translit($string);
        $string = str_replace(' ', '_', $string);
        return preg_replace('~([^a-z\.A-Z0-9_\(\)-])~', '', $string);
    }

    /**
     * Return the transliterated filename
     * @param string $filename
     * @return string
     */
    protected function translitFilename($filename) {
        $extAtPos = mb_strrpos($filename, '.');
        // transliterate filename without extension
        $name = $this->translit(mb_substr($filename, 0, $extAtPos));
        // include extension
        return $name.mb_substr($filename, $extAtPos);
    }
