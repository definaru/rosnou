...
class Generator extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $params = [
                ...
                'methods' => $this->generateMethods($tableSchema),
    ...
    public function generateMethods($table){

        $list = [];

        /*foreach ($this->generateRelations() as $modelName => $v) {

          if (stripos($modelName,"{$this->modelClass}Has")!==false){

            foreach($v as $modelLink => $v2){
                if ($modelLink!=$this->modelClass){

                  $list[] = "
    public static function modelNameLink(){
        return '{$v[$modelLink][1]}';
    }";
        ...
        foreach ($table->columns as $column) {
            ...
            if( substr($column->name, -6, 6) == '_index' ){
    $list[] = "
    function get" . Inflector::id2camel($column->name, '_') . 'List($index = null){
        $list = [
          1 => "Item 1",
          2 => "Item 2",
          3 => "Item 3",
    ...
    public function generateBehaviors($table){
        ...
        foreach ($table->columns as $column) {
            ...
            /*if( substr($column->name, -5, 5) == '_file' ){
                $list[] = "'{$column->name}' => [
                'class' => \\mongosoft\\file\\UploadImageBehavior::className(),
                'attribute' => '{$column->name}',
                'scenarios' => ['default'],
                'path' => '@uploadroot/". strtolower($this->modelClass) . "/{$column->name}/{id}',
                'url' => '@upload/". strtolower($this->modelClass) . "/{$column->name}/{id}',
            ]";
            }

            if( substr($column->name, -6, 6) == '_image' ){
                $list[] = "'{$column->name}' => [
                'class' => \\mongosoft\\file\\UploadImageBehavior::className(),
                'attribute' => '{$column->name}',
                'scenarios' => ['default'], // !!!
        ...
        foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
            ...
            if (stripos($table->name,"{$this->tableName}_has_")!==false
              /*AND $this->checkPivotTable($table) !== false*/){

                $pref = substr($table->name, strripos($table->name,"_has_")+5);
                $prefCamel = lcfirst( Inflector::id2camel($pref, '_') );

                $pref_plural = Inflector::pluralize($pref);

                $list[] = "['class'=>\\voskobovich\behaviors\ManyToManyBehavior::className(),"
                          ."'relations' => ['{$pref}_list' => '{$prefCamel}s'],]";
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $column) {
            ...
            switch ($column->type) {
                ...
                default: // strings
                    if ( substr($column->name, -6, 6) == '_image' ){
                        $images[] = $column->name;
                    } else if ( substr($column->name, -5, 5) == '_file' ){
                        $docs[] = $column->name;
                    } else if ( $column->name == 'mail' ||
                                $column->name == 'email'||
                                substr($column->name, -5, 5) == '_mail'||
                                substr($column->name, -6, 6) == '_email'
                    ){
        ...
        if( sizeof($images) > 0 ){
            $rules[] = "[['" . implode("', '", $images) . "'], 'string']";
            //$rules[] = "[['" . implode("', '", $images) . "'], 'file', 'extensions'=>'jpg, gif, png']";
        }

        if( sizeof($docs) > 0 ){
            $rules[] = "[['" . implode("', '", $docs) . "'], 'string']";
            //$rules[] = "[['" . implode("', '", $docs) . "'], 'file', 'extensions'=>'txt, html, htm, doc, docx, xls, xlsx, pdf']";
        }
        ...
        foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
            ...
            //$re = stripos($table->name,"{$this->tableName}_has_");
            //if( $re !== false ) {
                //var_dump($this->checkPivotTable($table)) . "\n";
            //}
    ...
    protected function generateRelations()
    {
        ...
        foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
            ...
            $link = $this->generateRelationLink([$field1 => $pks[1]]);
            $viaLink = $this->generateRelationLink([$pks[0] => $field0]);

            $relationName = $this->generateRelationName($relations, $className0, $db->getTableSchema($table0), $pks[1], true);
            ...
            // Relations for m2m
            $link = $this->generateRelationLink([$field0 => $pks[0]]);
            $viaLink = $this->generateRelationLink([$pks[1] => $field1]);
            $relationName = $this->generateRelationName($relations, $className1, $db->getTableSchema($table1), $pks[0], true);
    ...
    protected function checkPivotTable($table)
    {
        ...
        if (count($pk) !== 2 && count( $table->foreignKeys ) !== 2 ) {
            return false;
        }
        ...
        //print_r($table->foreignKeys);die;

        $fks = [];
        foreach ($table->foreignKeys as $refs) {

            $fieldName = array_keys($refs)[1];
            $fks[$fieldName] = array_values($refs);
