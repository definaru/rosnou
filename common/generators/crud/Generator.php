...
class Generator extends \yii\gii\generators\crud\Generator
{
    ...
    public function generateActiveField($attribute, $searchField = false)
    {
        ...
        // Генерим виджет m2m
        if ( !in_array($attribute, $attrs) && substr($attribute, -5, 5) == '_list' ){

            if( $searchField ){
                return null;
            }

            $class = substr($attribute, 0, -5);
            $linkedClass = '\common\models\\' . ucfirst( Inflector::id2camel($class, '_') );
        ...
        elseif ( $column->name == 'body' || $column->type === 'text' ){
            ...
            return "'$attribute'=>[
                ...
                //'options'=>['placeholder'=>yii::t('app', 'Enter').' ".$attributeLabels[$attribute]."...','rows'=> 6]
            ],";
        ...
        elseif ( substr($column->name, -3, 3) == '_id') {
            ...
            $foreignName = ucfirst(substr($column->name, 0, -3));
