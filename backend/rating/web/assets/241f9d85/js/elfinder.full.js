...
window.elFinder = function(node, opts) {
  ...
  var self = this,
    ...
    //
    this.showImageCompressDialog = function(files, bigFiles){

        function confirmSqueeze(opts) {
            var complete = false;
            var options = {
                cssClass  : 'elfinder-dialog-confirm',
                modal     : true,
                resizable : false,
                title     : self.i18n(opts.title || 'confirmReq'),
                buttons   : {},
                close     : function() {
            ...
            options.buttons[self.i18n(opts.accept.label)] = function() {
                opts.accept.callback();
                complete = true;
                $(this).elfinderdialog('close');
            };

            options.buttons[self.i18n(opts.cancel.label)] = function() {
                $(this).elfinderdialog('close');
            };

            return self.dialog('<span class="elfinder-dialog-icon elfinder-dialog-icon-confirm"/>' + self.i18n(opts.text), options);
        }

        confirmSqueeze({
            title : 'Большой файл',
            text  : 'Вы загружаете файлы размером более ' + (maxFileSizeAllowed / (1024 * 1024)).toFixed(1) + ' МБ:' +
            '<ul style="margin: 10px 0;padding: 0 20px;">' + bigFiles.join(' ') + '</ul>' +
            'Нажмите "Сжать", чтобы автоматически сжать указанные файлы.<br/><br/>' +
            'Если вы нажмете "Отмена", то ни один файл не будет загружен.',
            width : 410,
            accept : {  // accept callback - required
                label : 'Сжать',
                callback : function() {
                    console.log(files);
                    return self.transport.upload(files, self);
                }
            },
    ...
    this.showCommonSizeError = function(files){

        this.error('Вы загружаете файлы с недопустимым размером ' +
                   '(разрешены картинки до ' + maxImageSize/(1024*1024) + ' МБ, ' +
                   'документы до ' + (maxDocSize/(1024*1024)).toFixed(0) + ' МБ):' +
                   '<ul style="margin: 10px 0;padding: 0 20px;">' + files.join(' ') + '</ul>' +
                   'Файл необходимо уменьшить.');
    };

    this.correctExt = function(file){

        if( self.options.onlyExtensions.length == 0 ){
            return true;
        }

        if( self.hasExtension(file.name, self.options.onlyExtensions) ){
            return true;
        }
    ...
    this.correctMime = function(file){

        //console.log('file type:' + file.type + '; file mime: ' + file.mime);
        //console.log(file);

        var fileType = file.type;
        var mimeCategory = fileType.split('/').shift();

        if( self.options.onlyMimes.length == 0 ){
            return true;
        }

        if( $.inArray(fileType, self.options.onlyMimes) !== -1 ){
            return true;
        }

        if( $.inArray(mimeCategory, self.options.onlyMimes) !== -1 ){
            return true;
        }
    ...
    // показываем ошибку с некорректным расширением файла
    this.showExtError = function(extList){

      var extArray = [];

       $.map( extList, function(value){
          console.log(value);
          extArray.push(value);
       } );

       this.error('В данный раздел могут быть загружены ' +
           'только файлы следующих форматов: ' +
            extArray.join(', '));
    }

    // показываем ошибку с некорректным расширением файла
    this.showMimeError = function(allowedMimes){
       ...
       var typesMap = {
            ...
            '': 'xls', // xls - почему-то для xls определяется абсолютно пустой mime
       ...
       for (i in allowedMimes){

          if( typesMap[ allowedMimes[i] ] != undefined ){
              type = typesMap[ allowedMimes[i] ];
              extList[type] = type;
          }
       }
    ...
    this.hasExtension = function (fname, extList){
    ...
    this.isImageFile = function (fname) {

        if(!fname) return false;

        var parts = fname.split(".");
        var extension = parts[parts.length - 1].toLowerCase();
        var imageExt = ["gif", "jpg", "jpeg", "png", "bmp", "wbmp"];
        var result = $.inArray(extension, imageExt) == -1 ? false : true;

        console.log('isImage', fname, extension, result);

        return result;
    };

    // Проверяем что файл является pdf файлом
    this.isPdfFile = function(fname) {
        if(!fname) return false;
        var parts = fname.split(".");
        var extension = parts[parts.length - 1].toLowerCase();
  ...
  */
  ...
  this.upload = function(files) {
        ...
        if(files) {
            ...
            var self = this;
            var count = 0;
            var files_objects = null;

            // Кол-во файлов и массивы
            if(files.input) {
                if (files.input.files) {
                    count = files.input.files.length;
                    files_objects = files.input.files;
                }
            } else if(files.files) {
                count = files.files.length;
                files_objects = files.files;
            }

            console.log('count, files_objects', count, files_objects);

            // Если задано ограничение и превышено, выдаем предупреждение
            if(maxFileCountAllowed && count > maxFileCountAllowed) {
                this.error("Одновременно загружать можно не более " + maxFileCountAllowed + " файлов");
                return $.Deferred().reject();
            }

            // Если задан максимальный размер и имеются файлы на входе
            if(maxFileSizeAllowed && files_objects) {

                var bigFiles = [];
                var onlyImages = true;
                var isImage;

                //alert(maxFileSizeAllowed);

                // Проходим по массиву файлов
                for(var i=0; i<files_objects.length; i++) {

                    // проверка расширения файла
                    //if( !self.correctExt(files_objects[i]) ){
                    //    self.showExtError(self.options.onlyExtensions);
                    //    return $.Deferred().reject();
                    //};

                    // проверка типа файла
                    //if( !self.correctMime(files_objects[i]) ){
                    //    self.showMimeError(self.options.onlyMimes);
                    //    return $.Deferred().reject();
                    //};

                    console.log('Analyze file', files_objects[i].name);
                    isImage = self.isImageFile(files_objects[i].name);

                    onlyImages &= isImage;

                    // Если файл является картинкой и превышает допустимый рзмер
                    // добавляем сообщение в массив bigFiles
                    if(isImage && files_objects[i].size > maxImageSize) {
                        //bigFiles.push('<li style="margin: 0;padding: 2px 0;font-style: italic;">' + files_objects[i].name + '&nbsp;(' + (files_objects[i].size / (1024 * 1024)).toFixed(1) + '&nbsp;МБ)</li>');
                    }

                    // Если файл не является картинкой и превышает размер в 15 раз больше картинки
                    // добавляем сообщение в big files
                    if(!isImage && files_objects[i].size > (maxDocSize)) {
                        bigFiles.push('<li style="margin: 0;padding: 2px 0;font-style: italic;">' + files_objects[i].name + '&nbsp; (' + (files_objects[i].size / (1024 * 1024)).toFixed(1) + '&nbsp;МБ)</li>');
                    }
                }

                // Если имеются большие файлы
                // и в массиве файлов не только картинки
                // выводим сообщение
                if( bigFiles.length > 0 && !onlyImages ) {
                    self.showCommonSizeError(bigFiles, maxFileSizeAllowed);
                    return $.Deferred().reject();
                }

                // Если имеются большие файлы и массив состоит только из картинок
                if(bigFiles.length > 0 && onlyImages) {
                    return self.showImageCompressDialog(files, bigFiles);
                }
            }
        }
...
elFinder.prototype = {
  ...
  uploads : {
    ...
    iframe : function(data, fm) {
          ...
          .bind('load', function() {
            iframe.unbind('load')
              .bind('load', function() {
                ...
                console.log('before parseUploadData');
      ...
      //$.each(self.options.onlyExtensions||[], function(i, extension) {
        //form.append('<input type="hidden" name="extensions[]" value="'+extension+'"/>');
      //});
    ...
    xhr : function(data, fm) {
      ...
      xhr.addEventListener('load', function(e) {
        ...
        console.log('Before XHR parseUploadData');
      ...
      var send = function(files, paths){
        ...
        if (! dataChecked && (isDataType || data.type == 'files')) {
          ...
          //maxFileSize = maxFileSizeAllowed;//fm.option('uploadMaxSize')? fm.option('uploadMaxSize') : 0;
          ...
          for (var i=0; i < files.length; i++) {
            ...
            //console.log('before size check');
            //console.log( 'maxFileSize && files[i].size >= maxFileSize', '=', maxFileSize, '&&',files[i].size, '>=', maxFileSize );

           //  Проверка расширений файла
            //if( !self.correctExt(files[i]) ){
            //    self.showExtError(self.options.onlyExtensions);
            //    return $.Deferred().reject();
            //};

            //  Проверка типа файла
            //if( !self.correctMime(files[i]) ){
            //    self.showMimeError(self.options.onlyMimes);
            //    return $.Deferred().reject();
            //};

            // Определяем допустимый для данного файл размер
            var checkMaxFileSize = self.isImageFile(files[i].name)
                ? 0
                : maxDocSize;

            if (checkMaxFileSize && files[i].size >= checkMaxFileSize) {

              console.log('incorrect file size');

              self.error(
                  self.i18n('errUploadFile', files[i].name) +
                  ' ' +
                  self.i18n('errUploadFileSize') +
                  ' ('+ (checkMaxFileSize/(1024*1024)).toFixed(0)  +' МБ)'
              );
        ...
        //$.each(self.options.onlyExtension, function(i, extension) {
          //formData.append('extensions['+i+']', extension);
        //});
...
elFinder.prototype._options = {
  ...
  /**
   * Display only required files by extansions
   *
   * @type Array
   * @default []
   * @example
   *  onlyExtensions : ["doc", "docx", "xls", "xlsx"]
   */
  onlyExtensions : [],
...
if (elFinder && elFinder.prototype && typeof(elFinder.prototype.i18) == 'object') {
  elFinder.prototype.i18.en = {
    ...
    messages   : {
      ...
      'confirmMove'     : 'Are you sure you want to move files?',
      'confirmRename'   : 'Are you sure you want to rename file?',
  ...
  this.not('.elfinder-cwd').each(function() {
    ...
    var
      ...
      selectFile = function(hash) {
        ////
        return cwd.find('#'+hash).trigger(evtSelect);
      },
      ...
      scrollToView = function(o) {
        ...
        var ftop    = o.position().top,
          ...
          // -20 - поправка на горизонтальный скролл
          wheight = wrapper.innerHeight() - 20;
    ...
    fm
      ...
      .bind('mkdir mkfile duplicate upload rename archive extract paste multiupload', function(e) {
        ...
        $.each(e.data.added || [], function(i, file) {
          ...
          //// Действие с только что добавленным файлом
          if( file && file.phash == phash ){
              //;
              scrollToView(selectFile(file.hash));
          }
        });
    ...
    filter = function(files) {
      return $.map(files, function(file) {
        ...
        return (file.mime.indexOf('text/') === 0 || $.inArray(file.mime, mimes) !== -1)
          ...
          //&& (!self.onlyExtensions.length || self.hasExtension(file.name, self.onlyExtensions) )
